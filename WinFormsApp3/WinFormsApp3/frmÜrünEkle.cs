...
namespace WinFormsApp3
{
    public partial class frmÜrünEkle : Form
    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("UPDATE urun SET miktari = miktari + @Miktar WHERE barkodno = @Barkod", baglanti);

            // Parametreleri ekleyin
            komut.Parameters.AddWithValue("@Miktar", int.Parse(Miktarıtxt.Text));
            komut.Parameters.AddWithValue("@Barkod", BarkodNotxt.Text);
        ...
        private void BarkodNotxt_TextChanged(object sender, EventArgs e)
        {
            ...
            while (read.Read())
            {
                ...
                lblÜrünMiktari.Text = read["miktari"].ToString();
